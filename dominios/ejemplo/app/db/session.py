...
# ── SQLite: qué NO va a funcionar ────────────────────────────────────────────
#
# SQLite sirve para tests y dev local. No es un Postgres más chico: hay features que
# el dominio puede necesitar y que allá no existen.
#
#   JSONB e índices GIN  → usar JSON().with_variant(JSONB(), "postgresql"). El índice
#                          GIN se crea condicionalmente solo en Postgres.
#   Advisory locks       → pg_try_advisory_lock no existe. Si un dominio elige líder
#                          entre réplicas para correr jobs, ese camino no se puede
#                          testear en SQLite.
#   Índices parciales    → WHERE en un índice: SQLite los soporta, pero con sintaxis y
#                          cobertura distintas.
#   ON CONFLICT          → el upsert difiere.
#   ALTER TABLE          → muy limitado; las migraciones que renombran o cambian tipos
#                          necesitan batch_alter_table de Alembic.
#
# Consecuencia práctica: un test que pasa en SQLite puede fallar en Postgres. La
# migración de la verdad se valida contra Postgres, no contra SQLite.
#
# Y con base compartida entre dominios: SQLite tiene un write-lock global por archivo,
# así que varios dominios escribiendo el mismo .db se serializan entre sí. Anda para un
# dev y para tests; no refleja el comportamiento real bajo concurrencia.
